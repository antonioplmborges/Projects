/*
 * @topic T0L133 Matrix transformations
...
#define    BCC_PROJECT_NAME "Lecture13_Matrix"
...
#define BCC_USING_INDEX_BUFFER 1
    ...
    #define NUM_VERTICES num_vertices    // UPDATE!  Note, these values are in the .h file too
...
void render() {
    ...
    theta += 0.01f;
    scale_amount = 0.1f; // for phlegm model
    //scale_amount = sin(theta)*10.0f; // for teapot model
    //scale_amount = sin(theta)*1.5f; // for cube2 model
...
#endif
    glutSwapBuffers();
    glutPostRedisplay();        // UPDATE! This calls "render" again, allowing for animation!
}

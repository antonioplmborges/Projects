...
in vec3 fE;                // UPDATE!  Coming in from the vertex shader
...
void main () {
    ...
    vec3 E = normalize(-fE);    // UPDATE!    Reverse E
    vec3 H = normalize(L + E);    // UPDATE! Create the half vector    

    // Diffuse component
    float diffuse_intensity = max(dot(N, L), 0.0);
    //vec4 diffuse_final = diffuse_intensity*vec4(0.1, 0.8, 0.1, 1.0);        // UPDATE! Multiply the diffuse intensity times a color
